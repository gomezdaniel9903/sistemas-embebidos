...
KP = 0.4                # fuerza de correccion del seguimiento (ajustar)

INTERVALO_LOG = 0.5     # segundos entre filas del CSV
INTERVALO_BAT = 2.0     # segundos entre lecturas de bateria (es lenta)

# Llamadas al sensor de linea (via la puerta trasera run/eval de la libreria).
# Si tu firmware usa otros nombres, montalos en bloques y con "ver Python" en
# mBlock copias la sintaxis exacta (p. ej. cyberpi.quad_rgb_sensor_1.*).
SET_LINE_MODE = "mbuild.quad_rgb_sensor.set_line_track_mode()"
GET_OFFSET    = "mbuild.quad_rgb_sensor.get_line_track_offset()"
...
def leer_offset(bot):
    """Lee la desviacion respecto a la linea (-100..100). None si falla."""
    try:
        return float(bot.eval(GET_OFFSET))
    except Exception:
        return None
    ...
    try:
        ...
        while estado["ejecutando"]:
            ...
            # ---------- MODO PISTA (linea, velocidad baja constante) ----------
                ...
                off = leer_offset(bot)
                if off is None:
                    bot.stop()               # linea perdida o lectura fallida
                    offset = "perdida"
                else:
                    corr = off * KP
