...
BASE_PISTA = 20         # velocidad baja y constante para el modo pista
KP = 6.0                # fuerza de correccion (el error va de -3 a +3; prueba 5-8)

INTERVALO_LOG = 0.5     # segundos entre filas del CSV
INTERVALO_BAT = 2.0     # segundos entre lecturas de bateria (es lenta)

# Sensor de linea via la puerta trasera run/eval. En este firmware la funcion
# de offset devuelve None, pero is_line() funciona bien, asi que calculamos la
# desviacion nosotros a partir de las 4 pastillas (ver leer_error_linea).
SET_LINE_MODE = "mbuild.quad_rgb_sensor.set_line_track_mode()"
IS_LINE = "mbuild.quad_rgb_sensor.is_line({})"   # {} = numero de pastilla (1..4)
...
def leer_error_linea(bot):
    """Desviacion respecto a la linea calculada con las 4 pastillas.
    Negativo = linea a la izquierda, 0 = centrado, positivo = a la derecha.
    Devuelve None si se pierde la linea (todo blanco)."""
    try:
        # pastilla 1 = extremo derecho ... 4 = extremo izquierdo
        p = [1 if bot.eval(IS_LINE.format(i)) else 0 for i in (4, 3, 2, 1)]
    except Exception:
        return None
    pesos = [-3, -1, 1, 3]          # izquierda (-) ... derecha (+)
    activos = sum(p)
    if activos == 0:
        return None                 # linea perdida (todo blanco)
    if activos == 4:
        return 0.0                  # todo negro (cruce/marca): sigue recto
    return sum(w * v for w, v in zip(pesos, p)) / activos
...
def bucle_control():
    ...
    try:
        ...
        while estado["ejecutando"]:
            ...
            # Al cambiar de modo: parar y resetear, y preparar el sensor si toca
            ...
            # ---------- MODO PISTA (linea, velocidad baja constante) ----------
            elif modo == "pista":
                error = leer_error_linea(bot)
                if error is None:
                    bot.stop()               # linea perdida (todo blanco)
                    offset = "perdida"
                else:
                    corr = error * KP
                    # Si dirige al reves, invierte el signo (usa -corr)
                    bot.drive(BASE_PISTA + corr, BASE_PISTA - corr)
                    offset = round(error, 1)
